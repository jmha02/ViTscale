...
# Baseline Continual Learning with ViT on Split-CIFAR100 (No Regularization)
...
def train_task(model, train_loader, task_id):
    """Train model on a single task (baseline - no regularization)"""
    model.train()
    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=0.05)
    criterion = nn.CrossEntropyLoss()
    
    print(f"\nTraining Task {task_id} (Baseline)")
    print("-" * 40)
...
def main():
    print("Baseline Continual Learning with ViT on Split-CIFAR100")
    print("=" * 60)
    ...
    print(f"\n{'='*80}")
    print("FINAL BASELINE CONTINUAL LEARNING RESULTS")
    print(f"{'='*80}")
    ...
    if len(all_results) > 1:
        ...
        # Show dramatic forgetting effect
    ...
    print("\nBaseline Continual Learning completed!")
    print("This shows the catastrophic forgetting problem without any mitigation.")
